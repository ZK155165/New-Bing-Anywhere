...
import { type Bing, type Extra, type CorePageData } from '@@/types'
...
export default () => {
  ...
  //   errorRetryCount: 1
  // })
  const data: CorePageData = {} as any
  // useEffect(() => {
  //   if (!error) return
  ...
  const [content, setContent] = useState<Partial<CorePageData['content']>>()
  const [links, setLinks] = useState<CorePageData['links']>({ list: [] })
  const [searchSuggestions, setSearchSuggestions] = useState<CorePageData['searchSuggestions']>({ list: [] })
  useEffect(() => {
    if (!data) return
